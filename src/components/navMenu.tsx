...
import { NavigationMenuList, NavigationMenu, NavigationMenuItem, NavigationMenuLink } from "./ui/navigation-menu"
    ...
    return (
        <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="lg:hidden hover:bg-red-700 hover:text-white text-white">
                    <Menu className="h-5 w-5" />
                    <span className="sr-only text-white">Toggle menu</span>
                </Button>
            </SheetTrigger>
            <SheetContent side="right" className="bg-red w-[240px] text-white font-semibold pt-1 sm:w-[300px]">
                <SheetTitle className="space-y-4 block py-2 px-4 text-lg font-bold text-white">Menu</SheetTitle>
            ...
            <NavigationMenuList>
                {
                    menuItems.map((menu) => (
                        <NavigationMenuItem className="red" key={menu.title}>
                            <NavigationMenuLink asChild className={`group inline-flex h-9 text-lg text-white w-max items-center justify-center rounded-md px-4 py-2 font-semibold hover:text-white focus:text-white focus-visible:ring-ring/50 outline-none transition-[color,box-shadow] focus-visible:ring-[3px] focus-visible:outline-1`}>
